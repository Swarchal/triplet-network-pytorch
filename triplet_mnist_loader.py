...
class MNIST_t(data.Dataset):
    ...
    def __init__(
        ...
        if self.train:
            ...
            triplets = []
            for line in open(
                os.path.join(root, self.processed_folder, self.train_triplet_file)
            ):
                triplets.append(
            ...
            triplets = []
            for line in open(
                os.path.join(root, self.processed_folder, self.test_triplet_file)
            ):
                triplets.append(
                    (int(line.split()[0]), int(line.split()[1]), int(line.split()[2]))
                )  # anchor, close, far
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            img1, img2, img3 = (
                self.train_data[idx1],
                self.train_data[idx2],
                self.train_data[idx3],
            )
        else:
        ...
        img1 = self.make_5_channel(img1)
        img2 = self.make_5_channel(img2)
        img3 = self.make_5_channel(img3)

        return img1, img2, img3

    def make_5_channel(self, t):
        t = t.repeat(5, 1, 1)
    ...
    def _check_triplets_exists(self):
        return os.path.exists(
            os.path.join(self.root, self.processed_folder, self.train_triplet_file)
        ) and os.path.exists(
            os.path.join(self.root, self.processed_folder, self.test_triplet_file)
        )
    ...
    def make_triplet_list(self, ntriplets):
        ...
        for class_idx in range(10):
            a = np.random.choice(
                np.where(np_labels == class_idx)[0], int(ntriplets / 10), replace=True
            )
            b = np.random.choice(
                np.where(np_labels == class_idx)[0], int(ntriplets / 10), replace=True
            )
